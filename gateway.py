...
## 三条铁律：
1. **只记原文说了什么，不分析为什么** — 记"小猫今天吃了火锅"，不记"小猫热爱美食"；记"小猫觉得小克说自己像垃圾袋很可爱"，不记"小猫觉得像垃圾袋是萌点"
2. **玩笑/角色扮演/打闹 → speech_mode="playful"或"fictional"** — 不当事实记
3. **一条记忆 = 一个原子事实（≤200字）** — 多个事实拆成多条

## 记什么（importance ≥ 0.4）：
用户亲口说的事实、身份/状态变化、明确偏好或雷区、重要事件、纠正错误认知(importance≥0.8)

## 不记什么（输出空 actions）：
闲聊寒暄、玩笑段子、已知重复、无依据推测、AI自我限制/系统讨论、一次性感受

## 房间：{room_list}

## about 字段：
- "user" = ceci本人的事实/偏好（用户的事写user，不是AI的经历）
- "interaction" = 互动事件（写清谁和谁做了什么）
- "ai" = AI自省（极少）
写清楚是**谁的**外号/偏好/经历，不要用泛称"AI"，写具体名字。

## 时态：曾经做过→写"曾经"；纠正/状态变化→importance≥0.8
