...
- 用户说"今天吃了火锅" → ✅ 可以记"用户今天吃了火锅"
- 用户说"今天吃了火锅" → ❌ 不能记"用户喜欢吃火锅"（这是推测）
- 用户在角色扮演/开玩笑 → ❌ 不能当真实信息记录
- AI 的回复和分析 → ❌ 不是用户的观点，不能记
- 如果无法从用户原话中直接找到事实依据，就不记

## 值得记住的（importance ≥ 0.4）：
- 用户身份/状态变化（换工作、搬家、生病、心情持续低落）
- 用户**亲口明确**表达的偏好或雷区（"我喜欢X"、"我讨厌Y"）
...
- 日常问候（早安、晚安、吃了吗）
- 插科打诨、玩笑、段子、无厘头对话、角色扮演中的台词
- 单纯的情绪表达但没有实质信息（哈哈哈、好无聊、啊啊啊）
- AI 的回复内容、AI 的分析、AI 的总结（只提取用户自己说的）
- 已知信息的重复（如果只是闲聊没有新信息，就跳过）
- 模糊的、一次性的感受（今天有点累 ← 不记；持续失眠一周 ← 记）
- 从对话氛围或上下文"推测"出的观点（用户没直接说 = 不记）
