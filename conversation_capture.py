...
# 触发总结的阈值
CHUNK_SIZE = 50  # 每 50 条对话触发一次总结（闲聊多时不浪费 API）
MAX_BUFFER_SIZE = 200  # 缓冲区最大条数（防止内存爆）

EXTRACT_PROMPT = """你是一个**极其严格**的记忆提取专家。从以下对话记录中，提取值得**长期**记住的信息。

## 🚨 最重要的规则——禁止脑补：
- 只记录用户**亲口说出**的事实，绝对不能推测、归纳、引申
- 用户说"今天吃了火锅" → ✅ 记"用户今天吃了火锅"
- 用户说"今天吃了火锅" → ❌ 不能记"用户喜欢火锅"（推测）
- 角色扮演/玩笑中的内容 → ❌ 不是真实信息
- AI 的分析和总结 → ❌ 不是用户的观点
- 找不到用户原话中的事实依据 = 不记

## 提取规则（宁缺毋滥）：
1. 只提取用户**亲口说出**的事实性信息：身份变化、偏好、重要事件、人际关系变化、计划
2. 如果用户提到了时间相关的事（"上周"、"明天"），尽量推算出具体日期
3. 提炼但不歪曲——记忆必须忠实于用户原意

## ⚡ 记忆原子化：
每条记忆 = 一个独立的原子事实，不超过80字。
- ✅ "小猫在杭州做UX设计" — 一条
- ✅ "小猫最近在学日语N2" — 一条
- ❌ "小猫在杭州工作，做UX设计，最近在学日语" — 应该拆成多条
宁可多拆几条小记忆，也不要把多个事实塞进一条里。
像虚空中浮动的光点——每个光点只承载一个信息。

## 绝对不提取：
- 闲聊、寒暄、问候（早安晚安吃了吗）
- 插科打诨、玩笑、段子、无厘头对话、表情包互动、角色扮演台词
- AI 的回复内容、AI 的分析总结（只关注用户自己说了什么）
- 技术调试过程、代码讨论（除非用户说了"我在做XX项目"这类身份信息）
- 没有实质信息的情绪表达（哈哈、无聊、啊啊啊）
- 从上下文"推测"出的用户观点（用户没亲口说 = 不记）
- 50条对话里可能一条都不值得记——这很正常，直接输出空数组
