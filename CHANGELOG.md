...
## 2026-07-11 中文召回修复 + 待办流动 + 梦境去循环

四个用户实测发现的深层问题：

- **中文召回半盲（提"妈妈"召不回母亲记忆）**：FTS5 默认分词器不切中文，整段中文当一个 token，关键词路对中文 query 全盲；"精确匹配"路只在向量+FTS 已有候选里打分，没有独立查询。修复：`database.cjk_like_search()` 把 query 切成 2 字滑窗做 LIKE 子串搜索，作为第 4 路进 RRF 融合。实测"我妈妈"已能召回母亲相关核心记忆。
- **老记忆缺向量永远隐形**：`_backfill_analysis` 只补打标不补向量。新增 daemon 步骤 `backfill_embeddings`（每轮 40 条）。
- **待办写死不流动**：resolved=False 的记忆会永远置顶在走廊/召回里（"目前从事新媒体运营"挂了一个月就是这个原因）。记忆医生新增待办复审：挂 14 天以上的待办用小模型对照当前画像判断，不再成立的自动摘牌（留年轮注记）。
- **梦境循环 + 身份混淆**：白天 AI 讲昨晚的梦→对话被摘要→摘要进今晚梦材料→昨天的梦钉进今天（无限循环）。修复：梦材料剔除含"梦见/做梦/梦里"的摘要；昨晚的梦作为"禁止重复"负面清单传给模型。身份规则加固：同伴的模型出身/公司/封号遭遇不是自己的，梦里的"我"从头到尾只能是自己。
- **公开群隐私墙 + 限时任务过期**（同日早些时候）：public_group/big_group 不再注入完整走廊/待办/敏感房间记忆，只给轻量身份锚点+"不提主人私事"纪律；带 event_date 的轻记忆过期 2 天自动归档。
- **数据修复**：三条错挂"未解决"的旧职业/情绪记忆已摘牌，"目前从事新媒体运营"改写为过时标注，career 房新增现状记忆（行政/职能，非新媒体）。
...
- **`[记住:内容]`**：把用户提到的重要信息存入记忆
- **`[更新状态:分类=内容]`**：直接更新当前状态画像（职业/健康/生活）
- **`[这是谁:昵称=说明]`**：登记新人物到人物注册表
- **`[忘记:描述]`**：归档不再需要的旧记忆
- **`[draw:描述]`**：画图（已有功能纳入统一管理）

proxy.py 在 AI 回复收集完毕后（流式/非流式）统一扫描标签、执行对应 handler、从用户可见文本中清除标签。非流式回复还会把执行结果写入 `capability_results` 字段。能力提示自动注入 system prompt，替换了之前单独的 DRAW_HINT。

新增 API：`GET /api/capabilities` 列出所有能力，`POST /api/capabilities/test` 手动测试。

## 2026-07-10 记忆合并防漂移 + 走廊去重

修复"真丝裤衩→真丝羽毛"类型的记忆漂移问题。

- **合并 prompt 注入人物速查**：daemon 合并相似记忆和客厅整理时，现在把 `identity_registry.glossary_text()` 注入合并 prompt，让小模型知道谁是谁。
- **归属保护规则**：合并 prompt 明确要求"保留主语归属，不要把A的特征/行为写到B身上"，防止 Jasper 的真丝裤衩被合并成小克的真丝羽毛。
- **走廊字符去重**：corridor 的"关于主人"和"人物/关系索引"板块新增基于字符重叠比的去重（阈值 75%），解决走廊里三条几乎一样的"AI身份区分"占位问题。
- **已漂移的记忆需手动修正**：旧的错误合并结果已经存入数据库，这些需要在前端记忆库里找到并手动修改内容。

## 2026-07-10 人物速查去掉"是AI不是人类"强调

用户反馈 glossary 里对 AI 同伴追加"这是一个 AI，不是人类"没有必要且过度强调。改为"你的同伴，独立的另一个角色，不是你"——保留"不要混淆身份"的核心信息，去掉刻意的 AI/人类判词。
