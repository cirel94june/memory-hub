...
def get_model_profile(ai_id: str) -> dict:
    ...
    canonical = AI_ALIASES.get(ai_id, ai_id)
    aliases = AI_ALIAS_GROUPS.get(canonical, AI_ALIAS_GROUPS.get(ai_id, [ai_id]))
    # 小克 is displayed as "cloudy" in the app, but Web/MCP/API config is kept
    # under "claude". Let cloudy inherit claude's model settings first, while
    # get_profile() still prefers cloudy for the user-facing persona.
    if canonical == "claude" and ai_id == "cloudy" and "claude" in aliases:
        ordered = ["claude"] + [a for a in aliases if a != "claude"]
    else:
