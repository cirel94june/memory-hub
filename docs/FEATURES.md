...
- 群聊：用户发消息后，群成员里的真实社交 AI 会按 @、被回复目标或随机围观回复；支持消息级回复、删除、`@` 指定成员、一跳 AI-to-AI @ 回复，并进入 `private_group` 记忆缓冲。
...
| **夜间梦境日记** | 当天同一 AI 至少 2 条 `chat_digests`，当天未生成过梦 | 新增 `layer=private`、`room=diary`、`category=dream`、`source_platform=daemon_dream` 的梦境日记 | 梦境入口、skip log、Dream Context 浮现/注入开关；考虑与 `room=dreams` 合并展示 |
