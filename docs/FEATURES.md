...
- 记忆详情页会把 comments 作为年轮评论单独展示，并提供追加年轮入口。
- history 只代表合并/编辑的版本历史，不再混叫年轮，避免误以为年轮没有写入。
...
- 2026-09-22 已精简为 21 个工具（15 核心 + 6 过渡别名）；若客户端工具数不对，断开并重新连接 MCP。
...
### 梦境调性识别（2026-07-07）
- 夜梦 prompt 增加调性判断：恶作剧/捣乱/调侃/排查混乱/疲惫/温柔等都可以成为梦的主调，避免所有梦都被小模型写成温柔治愈。
