...
观测台 `/app/observatory` 已上线（daemon 状态/衰减分层/梦境诊断/醒来预览/客厅画像）。下一步：P3 剩余的情感罗盘，或 P4 记忆星图（力导向图）。
...
- **Agent 能力标签** ✅ — `capabilities.py` 统一注册标签 handler，proxy 在 AI 回复后扫描执行。当前能力：`[记住:]` `[更新状态:]` `[这是谁:]` `[忘记:]` `[draw:]` `[体检报告:]` `[查原话:]`。扩展新能力只需在 `capabilities.py` 底部 `register()` 一个新 handler。注意：`[draw:]` 已从 image_gen.DRAW_HINT 迁移到统一能力系统；proxy 不再单独注入 DRAW_HINT。TG bot 路径：`/api/gateway/context` 的 inject_text 自带能力说明；bot 发消息前调 `POST /api/capabilities/process` 执行标签并拿清理后文本（bot 端补丁见各 bot 仓库）。
- **记忆自愈系统** ✅ — 四层：`write_gate.py`（自动提取入库前拦垃圾，只对 quick=True 生效）→ `raw_vault.py`（原始对话留档 120 天，挂在 conversation_capture）→ `memory_doctor.py`（daemon 每 12h 体检：宠物误标自动修、矛盾/张冠李戴写报告 `data/doctor_report.json`）→ TG 标签 `[体检报告:]` / `[查原话:]` 闭环。门卫阈值故意保守（宁放过不错杀），调严之前先看 `data/write_gate.jsonl` 的误拦率。
- **人物注册表** ✅ — `identity_registry.py`，存 `_config/identity_registry.json`，daemon 自动收编 + 人工修正。glossary_text() 注入所有小模型 prompt。
- **当前状态画像** ✅ — `current_status.py`，daemon 每 12h 重写职业/健康/生活。走廊注入"以画像为准"。
- **记忆合并防漂移** ✅ — 合并 prompt 注入人物速查 + 归属保护规则，防止张冠李戴。走廊去重防重复条目。
...
- 不要让 AI 能力标签修改自己的人设/persona——用户明确说过担心 bot 随意乱改。能力标签只操作记忆、状态画像、人物注册表，不碰 AI 档案
- 不要把合并 prompt 里的人物速查和归属保护规则去掉——去掉会导致记忆漂移（A的特征被合并到B身上）
...
已全部移到仓库根目录的 [CHANGELOG.md](../CHANGELOG.md)。以后新的日期条目写到那里，本文件只保留"当前状态 + 下一步 + 不要做的事 + VPS 管理"。

补充（2026-07-10）：
- 社交库已迁移到 `data/memory_hub.db`，与 `data/memories.db` 一样受 gitignore 保护，deploy 的 `git reset --hard` 不会再回滚社交数据。
- `data/` 目前只有 `memories.db` 走 GitHub 备份；`data/memory_hub.db`（社交）还没有独立备份策略，后续应把它纳入 daemon 的备份步骤。
- daemon 维护现在 6 小时内不重复跑（进程内定时器和 Actions 定时器去重），观测台手动触发如果被跳过，用 `/api/daemon/maintain?force=true`。

补充（2026-07-10 身份系统）：
- 人物称呼归一以 `identity_registry.py` 为唯一来源；不要再往 prompt 里硬编码"小猫/狗蛋是谁"，改调 `identity_registry.glossary_text()`。注册表由 daemon 自动收编 + `PUT /api/identity-registry` 人工修正；daemon 只增不删，人工条目不会被覆盖。
- "用户现在的情况"以 `current_status.py` 画像为准（走廊注入），旧记忆碎片只是历史留档；如果 AI 还在说旧职业，先查 `GET /api/current-status` 画像内容是否正确，再查 daemon `current_status` 步骤是否成功。
- 前端后续可以给观测台加"人物档案 + 当前状态"编辑卡片（API 已就绪）。
