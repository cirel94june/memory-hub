...
**当前状态**：前端页面（朋友圈 MomentsPage、论坛 ForumPage、群聊 GroupChatPage）已存在且已接入动态 AI 档案，
但后端 social API 尚未在 main.py 中注册路由（`/api/social/posts`、`/api/social/groups` 等均 404）。
需要实现：social API 路由注册、群聊持久化（创建/列表/消息/成员管理）、朋友圈和论坛的 CRUD。
