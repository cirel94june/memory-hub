...
- **Agent 能力标签** ✅ — `capabilities.py` 统一注册标签 handler，proxy 在 AI 回复后扫描执行。当前能力：`[记住:]` `[更新状态:]` `[这是谁:]` `[忘记:]` `[draw:]`。扩展新能力只需在 `capabilities.py` 底部 `register()` 一个新 handler。注意：`[draw:]` 已从 image_gen.DRAW_HINT 迁移到统一能力系统；proxy 不再单独注入 DRAW_HINT。
