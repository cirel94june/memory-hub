...
- 群聊 AI 回复同样走 `_social_call_llm()`，会读取角色档案、走廊和相关记忆；回复后通过 `conversation_capture` 以 `private_group` 方式进入社交共享上下文。群聊支持回复某条消息、删除消息、`@` 指定成员；AI 回复里的 `@` 只作为文字保留，不再自动触发第二轮，避免重复连锁回复。
