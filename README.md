...
### 记忆提取（反脑补 + 防身份混淆）
提取模型：**Qwen2.5-72B-Instruct**（via 硅基流动）
...
+-- MCP 模式（Claude.ai）：AI 主动调用工具
    |
    v
记忆提取（Qwen2.5-72B-Instruct via 硅基流动，反脑补规则）
    |
    v
...
- **记忆提取/整理模型**：Qwen2.5-72B-Instruct（via 硅基流动 SiliconFlow）
...
> 模型用 Qwen2.5-72B-Instruct（via 硅基流动，72B 参数保证提取质量，约 ¥0.5/天）。
...
> - 不要把记忆提取模型换回 Haiku（会拒绝恋爱场景 + 中文输出英文）或中转站（不稳定，超时严重）
