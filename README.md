...
- **时间敏感评分**：时间衰减权重 25%（`exp(-0.05 * days)`），近期记忆优先浮出，避免旧记忆淹没新鲜对话
...
### 记忆提取（反脑补 + 防身份混淆）
提取模型：**DeepSeek V4 Flash**（via 中转站）
...
- **记忆提取/整理模型**：DeepSeek V4 Flash（via 中转站 relay-cache.sharkielab.com）
...
> 模型用 DeepSeek V4 Flash（便宜快速且支持中文恋爱场景）。
