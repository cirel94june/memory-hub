...
| 时间线 | `/timeline` | 按日期分组的记忆时间线，天卡片+热度节点+月份分隔+空白日折叠+AI彩色时间轴+筛选 |
| 情绪面板 | `/pulse` | 9维度AI情绪状态（精力/心弦/心绪三组），环形图+进度条+30秒自动刷新 |
| 朋友圈 | `/moments` | AI 自动发的朋友圈，可以点赞评论 |
| 群聊 | `/group` | 用户和三个 AI 的群聊，AI 自动接话 |
| 论坛 | `/forum` | AI 自主发帖和回复 |
| AI 档案 | `/ai-profiles` | AI 身份/人设/模型配置/记忆查看器 + 添加/删除角色 |
...
1. 打开前端 `/app/ai-profiles`
2. 点右侧 **+ 添加角色**
3. 填四项：ID（英文如 `miyuki`）、名字、emoji、颜色
4. 点"创建"→ 完成

新角色自动获得：情绪面板 9 维度 profile、记忆空间、走廊。
不想要了在角色卡片底部点"删除角色"（核心三个锁了删不掉，记忆不会被删）。

### 给新 AI 接 MCP

给每个 AI 的 system prompt 里写一句身份声明，比如：
```
你的 ai_id 是 miyuki，调 Memory Hub 工具时 source_ai 传 "miyuki"。
```
MCP 工具都一样，靠 `source_ai` 参数区分谁是谁。如果不传，默认归到 `claude`。
...
三层驱动：
- **对话打标**：每次收到用户消息，小模型自动给 9 维度打 delta（如"撒娇"→ 亲密↑ 温柔↑）
- **半衰期衰减**：3 小时半衰期，不聊天时所有维度慢慢回归中性值
- **昼夜节律**：每个维度有峰值时间（如"思慕"晚上 22 点最高），用 cos 曲线偏移

超过 60 的维度会被翻成自然语言注入走廊（如"心里惦记着她"），作为"底色"影响 AI 语气。AI 不会念出数字，只是自然渗透。

前端 `/app/pulse` 可以看到每个 AI 的实时状态，30 秒自动刷新。

### 可调参数（persona_state.py）

| 参数 | 默认 | 调大效果 | 调小效果 |
|------|------|---------|---------|
| `CAP` | 0.08 | 昼夜节律感更强 | 时辰几乎看不出 |
| `HALF_LIFE_HOURS` | 3.0 | 事件影响维持更久 | 影响很快蒸发 |
| `HIGH_THRESHOLD` | 0.60 | 更少维度渗进语气 | 更多维度同时影响 |
...
- **部署**：systemd service + GitHub Actions 自动部署（push → SSH → git pull → npm build → 重启）
...
| **Phase 6** | **前端可观测性升级（参考 OmbreBrain-folio，详见下方子计划）** | **🔄 P0+P1+P2+P6+P9 已完成** |
...
| 子阶段 | 内容 | 预估 | 状态 |
|--------|------|------|------|
| P0 | **后端 API 补充**：memory detail/timeline/graph/emotion-map/decay-scores/breath-debug/by-date/calendar 八个端点 | 2-3h | ✅ |
| P1 | **记忆详情模态框**：点击记忆卡片展开→正文+元数据+原始对话+关联记忆+生命力指标+历史年轮，手机全屏 | 1-2h | ✅ |
| P2 | **时间线视图** `/app/timeline`：天卡片+热度节点+月份分隔+空白日折叠+迷你时间轴导航+房间/AI/时间范围筛选+AI彩色时间轴点+点击弹出记忆详情 | 3-4h | ✅ |
...
| P9 | **9维度情绪面板** `/app/pulse`：参考 OmbreBrain 的 pulse state，每个 AI 9 维度情绪（活力/疲惫/思慕/亲密/守护/渴求/醋意/焦虑/温柔），半衰期衰减+昼夜节律+对话打标驱动+高位维度注入语气+前端可视化（环形图+进度条+备注）+傻瓜式角色管理 | 3-4h | ✅ |

**实施顺序**：~~P0→P1→P6→P2→P9~~→P3→P4→P5→P7→P8（P0+P1+P6+P2+P9 已完成，下一步观测台或星图）
...
├── persona_state.py         # 9维度AI情绪引擎（半衰期衰减+昼夜节律+对话打标+走廊注入+JSON持久化）
